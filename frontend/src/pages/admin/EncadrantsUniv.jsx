...
const EMPTY = { nom: '', prenom: '', mail: '', tel: '', specialite: '', grade: '' };
const EDITABLE = ['nom', 'prenom', 'mail', 'tel', 'specialite', 'grade'];

export default function EncadrantsUniv() {
  ...
  const fetchList = useCallback(async () => {
    ...
    try { setList(await api.encadrantsUniv.byUniversite(universiteId)); }
    catch (e) { setError(e.message); } finally { setLoading(false); }
  }, [universiteId]);

  useEffect(() => { if (universiteId) fetchList(); }, [universiteId, fetchList]);

  const handleAdd = async (e) => {
    e.preventDefault(); setAddError(''); setAddLoading(true);
    try {
      await api.encadrantsUniv.create({ ...addForm, id_universite: universiteId });
  ...
  const deleteOne = async (id) => {
    if (!window.confirm(t('supervisors.deleteConfirm'))) return;
    try { await api.encadrantsUniv.remove(id); setList((p) => p.filter((e) => e.id !== id)); toast.success(t('common.deleted', 'Supprimé')); }
    catch (err) { toast.error(err.message); }
  };
  const deleteMany = async (ids) => {
    if (!window.confirm(t('supervisors.deleteManyConfirm'))) return;
    try { await Promise.all(ids.map((id) => api.encadrantsUniv.remove(id))); setList((p) => p.filter((e) => !ids.includes(e.id))); toast.success(t('common.deleted', 'Supprimé')); }
  ...
  const saveEdit = async () => {
    try {
      await api.encadrantsUniv.update(viewing.id, editForm);
      await fetchList();
  ...
  const columns = [
    ...
    { key: 'grade', label: t('profile.grade'), searchable: true },
  ];
        ...
        <div className="card max-w-6xl mx-auto">
          ...
          {showAdd && (
            <form onSubmit={handleAdd} className="mb-6 p-5 rounded-xl grid grid-cols-1 sm:grid-cols-2 gap-4" style={{ background: 'var(--muted-bg)' }}>
              {[['nom', t('profile.nom'), true], ['prenom', t('profile.prenom'), true], ['mail', t('auth.email'), true], ['tel', t('profile.tel'), false], ['specialite', t('profile.specialite'), false], ['grade', t('profile.grade'), false]].map(([n, l, req]) => (
                <div key={n}>
